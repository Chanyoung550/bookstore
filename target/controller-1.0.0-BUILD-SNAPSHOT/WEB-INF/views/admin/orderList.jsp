                        ...
                        <tbody>
                           <c:forEach items="${list}" var="list" varStatus="vs">
                              <tr id="">
                                 ...
                                 <input type="hidden" id="oNum${vs.getIndex()}" value="${list.od_num}">
                                 <c:set var="result" value="${list.od_dv}"/> <!-- 배송상테 변수 설정  -->
                                 <c:choose>
                                    <c:when test="${result == '0'}"> <!-- '0'이면 배송준비중 -->
                                       <input type="button" id="confirm${vs.getIndex()}" class="btn btn-outline-secondary" value="배송준비중">
                                    </c:when>
                                    <c:when test="${result == '1'}"> <!-- '1'이면 배송중 -->
                                       <input type="button" id="confirm${vs.getIndex()}" class="btn btn-outline-secondary" value="배송중">
                                    </c:when>
                                    <c:otherwise> <!-- 나머지 배송완료 -->
